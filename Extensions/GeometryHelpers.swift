...
public extension CGSize {
    ...
    /// A `CGRect` with this size and the given origin.
    func rectangle(at origin: CGPoint = .zero) -> CGRect {
        return CGRect(origin: origin, size: self)
    }

    /// A `CGRect` with this size and the given center.
    func rectangle(centeredAt center: CGPoint) -> CGRect {
    ...
    var roundedDown: CGSize {
        return floor(self)
    }

    /// A size capped to be no greater in either dimension than `maxDimensions`.
    func capped(toAtMost maxDimensions: CGSize) -> CGSize {
    ...
    func capped(toAtLeast minDimensions: CGSize) -> CGSize {
    ...
    static func squareSize(toFillWidth containerWidth: CGFloat, withNoFewerThan minimumCellsPerRow: Int, cellsWiderThan minimumCellWidth: CGFloat, spacedApartBy spacing: CGFloat = 0, edgeMargin: CGFloat = 0) -> CGSize {
...
public extension CGRect {
    ...
    /// margin of `dx`.
    mutating func move(rightOf sibling: CGRect, margin dx: CGFloat = 0) {
        origin.x = sibling.maxX + dx
    }

    /// Move `origin.x` so that the frame is to the left of `sibling` with a
    /// margin of `dx`.
    mutating func move(leftOf sibling: CGRect, margin dx: CGFloat = 0) {
    ...
    mutating func move(insideTopEdgeOf parent: CGRect, margin dy: CGFloat = 0) {
        origin.y = dy
    }

    /// Move `origin.y` so that the frame is aligned to the bottom edge of `parent`
    /// _in its coordinates_, with a margin of `dy`.
    mutating func move(insideBottomEdgeOf parent: CGRect, margin dy: CGFloat = 0) {
        origin.y = parent.height - height - dy
    }

    /// Move `origin.x` to the horizontal center of `sibling`, with an offset of `dx`.
    mutating func move(centeredHorizontallyWith sibling: CGRect, offset dx: CGFloat = 0) {
        origin.x = (sibling.midX - (width / 2)) + dx
    }

    /// Move `origin.y` to the vertical center of `sibling`, with an offset of `dy`.
    mutating func move(centeredVerticallyWith sibling: CGRect, offset dy: CGFloat = 0) {
    ...
    mutating func move(centeredHorizontallyInside parent: CGRect, offset dx: CGFloat = 0) {
    ...
    mutating func move(centeredVerticallyInside parent: CGRect, offset dy: CGFloat = 0) {
...
public extension CGPoint {
    ...
    func moved(right dx: CGFloat) -> CGPoint {
        return CGPoint(x: x + dx, y: y)
    }

    /// Move the point right by `dx`.
    mutating func move(right dx: CGFloat) {
        x += dx
    }

    /// The point moved left by `dx`.
    func moved(left dx: CGFloat) -> CGPoint {
        return CGPoint(x: x - dx, y: y)
    }

    /// Move the point left by `dx`.
    mutating func move(left dx: CGFloat) {
        x -= dx
    }

    /// The point moved down by `dy`.
    func moved(down dy: CGFloat) -> CGPoint {
        return CGPoint(x: x, y: y + dy)
    }

    /// Move the point down by `dy`.
    mutating func move(down dy: CGFloat) {
        y += dy
    }

    /// The point moved up by `dy`.
    func moved(up dy: CGFloat) -> CGPoint {
        return CGPoint(x: x, y: y - dy)
    }

    /// Move the point up by `dy`.
    mutating func move(up dy: CGFloat) {
        y -= dy
    }
}
...
public extension CGAffineTransform {
    ...
    static let flipVertically = CGAffineTransform(scaleX: 1, y: -1)

    /// A transform that mirrors the horizontal axis.
    static let mirrorHorizontally = CGAffineTransform(scaleX: -1, y: 1)
}
